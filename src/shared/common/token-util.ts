...
export function getPowerEstimation(
  ...
  if (amount > 0) {
    const percent =
      (daysLeft > 0 ? Math.log(daysLeft) / Math.log(1.2) / 100 : 0) + 1;
...
export function getPowerEstimationForKovan(
  ...
  if (amount > 0) {
    const percent =
      (hoursLeft > 0 ? Math.log(hoursLeft) / Math.log(1.2) / 100 : 0) + 1;
    total = new BigNumber(amount).multipliedBy(percent);
  }
  const aDate = new Date();
...
export class Bucket {
  ...
  withdrawWaitUntil: string | 0;
  // tslint:disable-next-line:variable-name
  isPatch_: boolean = false;
